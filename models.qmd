...
## Introduction
The models employed in this project encompassed the Bayesian model and the Ordinal model. The Bayesian model, specifically the stan_glm framework, served as a tool for estimating the likelihood of the covariates (namely, literacy, infant mortality, and agriculture) exerting influence on the degree of endangerment. Conversely, the ordinal model, utilizing the stan_plor approach, made the prediction of the endangerment probability associated with each classification level of endangerment.
